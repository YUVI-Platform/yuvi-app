...
export default function StepSlotPicker({
  ...
  return (
    <section className="space-y-3">
      ...
      {/* Legende: mobile = einklappbar, ab lg dauerhaft sichtbar */}
      <details className="lg:hidden rounded-lg border bg-white p-3">
      ...
      <div className="relative">
        {/* Scroll-Buttons (ab md) */}
        <button
          type="button"
          onClick={() => scrollBy("left")}
          aria-label="nach links scrollen"
          className="absolute left-2 top-1/2 z-20 hidden -translate-y-1/2 rounded-full bg-white/90 p-2 shadow ring-1 ring-black/10 hover:bg-white md:block"
        >
          ‹
        </button>
        <button
          ...
          className="absolute right-2 top-1/2 z-20 hidden -translate-y-1/2 rounded-full bg-white/90 p-2 shadow ring-1 ring-black/10 hover:bg-white md:block"
        >
          ›
        </button>

        {/* Edge Fades für bessere Scroll-Affordance */}
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-white to-transparent md:w-10" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-white to-transparent md:w-10" />

        <div
          ref={scrollRef}
          className="relative overflow-x-auto overflow-y-hidden rounded-xl border bg-white p-3 min-h-[280px] snap-x snap-mandatory"
        >
            ...
            {days.map((d) => {
              ...
              return (
                <div
                  key={k}
                  className={clsx(
                    // Responsives Spaltenmaß:
                    // Mobile: ~70vw (fokussiert eine Tageskarte)
                    // sm: ~45vw, md: feste Breite
                    "snap-start min-w-[72vw] sm:min-w-[45vw] md:min-w-[300px] lg:min-w-[260px] max-w-[85vw] md:max-w-[320px] rounded-lg ring-1 p-2",
                    hasAnyAvail
                      ...
                      dayAvail.map((s) => {
                        ...
                        return (
                          <button
                            ...
                            className={clsx(
                              "w-full rounded-md px-2 py-2 text-[13px] ring-1 ring-inset text-left transition",
                              selected
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              {typeof s.capacity === "number" && (
                                <span
                                  className={clsx(
                                    "rounded bg-slate-100 px-1.5 py-0.5 text-[11px] leading-none",
                                    selected && "bg-white/20"
                                  )}
                                  aria-label={`Kapazität ${s.capacity}`}
                                >
                                  {s.capacity}
                                </span>
                              )}
...
/* ---------- Small sub-component: Legend items ---------- */
